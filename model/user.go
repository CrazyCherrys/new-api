...
func ReconcileUserImageGenerationActiveTaskCount(id int) (int64, error) {
	...
	var actual int64
	if actualCount, err := CountImageTasksByUserAndStatuses(id, activeStatuses); err != nil {
		return 0, err
	} else {
		actual = actualCount
	}
...
func ReconcileAllUserImageGenerationActiveTaskCounts() (int, error) {
	activeStatuses := []string{"pending", "generating"}
	rows, err := CountImageTasksGroupedByUserAndStatuses(activeStatuses)
	if err != nil {
		return 0, err
	}
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		for userID, count := range rows {
			if err := tx.Model(&User{}).
				Where("id = ?", userID).
				Update("image_generation_active_tasks", count).Error; err != nil {
				return err
			}
			corrected++
		}
