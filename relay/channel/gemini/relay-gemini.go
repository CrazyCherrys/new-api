...
// GeminiNativeImageHandler 解析 Gemini 原生 :generateContent 接口返回的图像生成响应
// （gemini-2.5-flash-image / gemini-3-pro-image-preview 等模型走这里）。
//
// 响应结构：
//
//	{
...
//	}
//
// 这里把所有 inlineData 部分提取出来，转成 OpenAI 格式 ImageResponse 的 b64_json 数组返回，
// 让上游统一的 /v1/images/generations 调用方拿到一致的响应结构。
func GeminiNativeImageHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	for _, candidate := range geminiResponse.Candidates {
		for _, part := range candidate.Content.Parts {
			if part.InlineData != nil && part.InlineData.Data != "" {
				...
				// 模型在生成图像时通常也会附带一段文本说明，记录到 revised_prompt 便于排查。
	...
	if len(openAIResponse.Data) == 0 {
		// 兜底：尽量把 prompt feedback / 阻断原因带回去，便于定位
		var blockReason string
		if geminiResponse.PromptFeedback != nil && geminiResponse.PromptFeedback.BlockReason != nil {
			blockReason = *geminiResponse.PromptFeedback.BlockReason
		}
		errMsg := "no images generated by gemini"
		if blockReason != "" {
			errMsg = fmt.Sprintf("%s (block_reason=%s)", errMsg, blockReason)
		}
		return nil, types.NewOpenAIError(errors.New(errMsg), types.ErrorCodeBadResponseBody, http.StatusInternalServerError)
	}
	...
	// 计费：优先采用上游 usageMetadata；缺失则按"每张固定 258 token"估算。
	usage := buildUsageFromGeminiMetadata(geminiResponse.UsageMetadata, info.GetEstimatePromptTokens())
