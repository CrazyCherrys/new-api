...
const ModelMappingTable = ({
  ...
  const isValidRequestEndpointForModelType = (modelType, endpoint) => {
    ...
    const endpointOptions = {
      ...
      2: ['openai', 'openai-response', 'gemini', 'openai_mod'],
      3: ['openai-video-generation', 'openai-video'],
    };
    return (endpointOptions[Number(modelType)] || []).includes(normalizedEndpoint);
  };
  ...
  const sanitizeMappingPayloadForUpdate = (record) => {
    ...
    if (
      modelType === 1 ||
      (modelType === 2 && payload.request_endpoint !== 'gemini') ||
